...
`cleanup-email` 是一个通过 IMAP 扫描并删除历史邮件的命令行脚本。主要用途是按配置中的规则批量清理旧邮件，同时对包含附件、回复、或来自重要发件人的邮件保留。
...
```json
{
  ...
  "important_sender": [".*@yourcompany\\.com"],
  "important_email_subjects": ["Invoice", "Receipt"],
...
- 生成一个可编辑的配置模板，或
- 将 `cleanup-email` 改造为支持 `--dry-run`（仅列出不删除），
