#!/usr/bin/env python3
"""Download all Yahoo Mail messages from one day as text JSON files."""
...
def main():
    parser = argparse.ArgumentParser(description="按日期下载 Yahoo 邮件为 JSON")
    parser.add_argument("email_address")
    parser.add_argument("date", help="日期，格式 YYYY-MM-DD")
    parser.add_argument(
        "-t",
        "--token-env",
        default="YAHOO_MAIL_TOKEN",
        help="存放 Yahoo token 的环境变量名，默认 YAHOO_MAIL_TOKEN",
    )
    parser.add_argument(
        "-p",
        "--print",
    ...
    try:
        ...
        logger.error("读取 Yahoo 邮件失败: %s", error)
        sys.exit(1)
